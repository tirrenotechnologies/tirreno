...
class Devices extends \Tirreno\Controllers\Services\Base {
    public function getList(int $apiKey): array {
        ...
        return $this->idMapIterate($map, tirreno('grids')->devices, $apiKey);
    }

    public function getChart(int $apiKey): array {
        return tirreno('charts')->devices->getData($apiKey);
    }

    public function getDeviceDetails(int $id, int $apiKey): array {
        $details = tirreno('models')->device->getFullDeviceInfoById($id, $apiKey);
    ...
    private function isEnrichable(int $apiKey): bool {
        return tirreno('models')->apiKeys->attributeIsEnrichable('ua', $apiKey);
    }
}
